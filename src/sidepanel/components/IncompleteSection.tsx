...
export default function IncompleteSection({ issues }: IncompleteSectionProps) {
  ...
  return (
    <section className="border-t border-border/40 shrink-0">
      <button
        type="button"
      ...
      {expanded && (
        <ul className="max-h-64 space-y-2 overflow-y-auto px-4 pb-4">
          {issues.map((issue) => (
            <li key={issue.id} className="rounded-md border border-border/50 bg-muted/10 p-3">
              <div className="mb-1 flex items-center gap-2">
                <Badge variant="minor">Review</Badge>
                <span className="text-mono text-[10px] tracking-wider text-muted-foreground">
