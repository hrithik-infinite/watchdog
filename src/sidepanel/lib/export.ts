...
export async function exportPDF(
  ...
  para(
    `Scanned ${formatDate(result.timestamp)}   -   Scan time ${formatDuration(result.duration)}   -   ${total} issue${total === 1 ? '' : 's'}${result.incomplete.length ? `   -   ${result.incomplete.length} to review` : ''}`,
  ...
  const generatedAt = toPdfSafeText(`Generated by WatchDog on ${formatDate(Date.now())}`);
...
`
  ...
  const footer = `
*Generated by WatchDog on ${formatDate(Date.now())}*
`;
